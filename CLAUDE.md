...
Frozen-sample baseline (`out/score_word_miss_baseline_v2.json`, `_SCORE_CACHE_VERSION` 4): overall `perfect_rate` 0.7021, mean 6.25, p90 1.0, penalized 11.9%, `geom_unconfident` 3.0%, `acc@1` 0.7957. By book — `haaji-agha` 0.882, `bachehaye_ghali` 0.744, `boof-e-koor` 0.364. By tier, the **`match` tier (usable Unicode text layer) is the weak one at 0.444 perfect / 22.2% degenerate**, well below `scan_vlm` at 0.847 / 2.4% — the image-scan path is the healthy one.

**This baseline replaced `out/score_word_miss_baseline.json` (0.6936 / 7.76 / p90 6.0 / `acc@1` 0.7787) because THE MEASURING INSTRUMENT CHANGED, NOT THE LOCATOR.** The judge had been reading correctly-placed boxes out against the wrong printed line (see `_split_tall_runs` below); repairing that re-graded 13 of 240 cases, 12 of which sat on a demonstrably drifted alignment before the fix. Do NOT read the rise as locator progress, and do NOT run `compare --gate` across the two: the gate is built for "instrument fixed, locator changed" and this is the reverse. Old reports remain readable on their old fields; they are not comparable on the new ones.
