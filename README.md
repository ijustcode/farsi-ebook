...
![farsi2epub pipeline architecture](architectural-diagram.png)
...
Boxes use two evidence sources: **blue** (`match`) for usable embedded PDF words and **green** (`scan`) for printed text recognized in image crops. Corrections use the full passage, with separate boxes across printed lines. Placement chips show pending, located, buffered, or unresolved; they describe evidence support, not independently audited accuracy. Unsupported locations have no box and remain editable. Legacy verifier coordinates are ignored.

Use `--bbox-mode auto|offline`, `--bbox-model MODEL`, and `--bbox-max-cost USD` (default $5 for this review session). Auto mode acquires missing readings with a bounded budget; offline mode replays compatible evidence and leaves missing locations unresolved. Successful readings are cached separately from derived geometry under `books/<slug>/locate_evidence/`. The old strip-reader prompt/schema is incompatible with this engine; old derived boxes are never relabeled as new results.

`--bbox-refine-model` is a deprecated alias for `--bbox-model`. `--bbox-refine` and `--no-bbox-refine` map to auto and offline with a migration message. Explicit `--bbox-refine-algorithm` requests now fail; historical algorithms are available only in offline evaluation tooling.

This consolidated engine is experimental pending independent held-out evaluation. See [implementation and evaluation status](docs/bbox-consolidation.md).
