...
Where a correction can be located on the page, a box is drawn on the page image, colored by how it was found: **blue** = matched word-for-word in the PDF's text layer, **teal** = located from PDF text-line geometry, **green** = located directly from image layout on a scanned page, **dashed orange** = the verifier's own estimate. A placement score appears beneath the correction controls: green `0` means the required words are inside the box, amber/red values show the word-distance of a miss, and `—` means the placement could not be verified. Hovering a correction highlights its box and vice versa. Clicking a correction (or its box) smoothly **zooms** the page image into that spot and marks the correction active; while zoomed you can **drag to pan**, and a plain click, `Escape`, or clicking the background zooms back out.

Scan refinement keeps successful strip readings separately from the boxes derived from them. Once a strip has been read, locator improvements can replay that evidence offline rather than calling the model again. `context_anchor_v1` is the default derivation; `--bbox-refine-algorithm legacy_v1` selects the superseded control for an explicit comparison. A pre-v4 `locate_vlm.json` contains only final boxes, not reusable readings, so the first v4 refinement of an uncached strip may still make one model call.
