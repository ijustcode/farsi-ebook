...
- The scan map currently requires readable text for every detected line. Unreadable lines, including noise mistaken for text, can make the page unresolved. Relaxing this requires independent non-text classification or stronger region evidence; local anchors alone cannot exclude a duplicate phrase in unreadable text.
